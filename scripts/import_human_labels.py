...
def _validate_reason_codes(reasons: Sequence[str], *, allow_unknown_reasons: bool) -> None:
    ...
    if unknown:
        raise ValueError(
            f"Unknown reason codes: {unknown}. Use --allow-unknown-reasons to bypass."
        )


def _validate_label_columns(columns: Sequence[str]) -> None:
    ...
    *,
    ...
    for case in cases:
        ...
        if parsed_pass and reasons:
            raise ValueError(
                f"Case {case.get('case_id')}: pass=true but fail reasons are not empty."
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import human labels (JSONL/CSV) and merge into benchmark cases JSONL"
    )
    parser.add_argument(
        "--cases",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Input labels file (JSONL or CSV) generated by export_benchmark_for_labeling.py",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print(f"Merged labels into {updated} case(s). Output: {output_path}")
