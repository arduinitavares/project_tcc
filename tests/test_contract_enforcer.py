...
class TestInvestValidationExpected:
    """Test invest_validation_expected parameter for A/B matrix semantics.
    
    When enable_spec_validator=False, missing validation_result should NOT
    produce a contract failure. Instead, is_valid should be None (unknown).
    """

    def test_validation_expected_true_missing_result_fails(self):
        """When validation is expected but missing, should fail with INVEST_RESULT_MISSING."""
        story = {
            "title": "Test story",
            "description": "As an automation engineer, I want to test so that quality improves.",
        ...
        result = enforce_story_contracts(
            ...
            theme="Core",
            epic="Testing",
            invest_validation_expected=True,  # Default behavior
        )
        
        assert result.is_valid is False
        assert result.validation_skipped is False
        rules = [v.rule for v in result.violations]
        assert "INVEST_RESULT_MISSING" in rules

    def test_validation_expected_false_missing_result_returns_none(self):
        """When validation is NOT expected (disabled), missing result should return is_valid=None."""
        story = {
            "title": "Test story",
            "description": "As an automation engineer, I want to test so that quality improves.",
            "acceptance_criteria": "- Test passes",
        ...
        result = enforce_story_contracts(
            ...
            theme="Core",
            epic="Testing",
            invest_validation_expected=False,  # Validation was intentionally disabled
        )
        
        # Key assertion: is_valid should be None (unknown), NOT False
        assert result.is_valid is None
        assert result.validation_skipped is True
        assert len(result.violations) == 0

    def test_validation_expected_false_other_violations_still_fail(self):
    ...
    def test_validation_expected_false_with_present_result_passes(self):
        """When validation is disabled but result is present anyway, should pass."""
        story = {
            "title": "Test story",
            "description": "As an automation engineer, I want to test so that quality improves.",
            "acceptance_criteria": "- Test passes",
        ...
        result = enforce_story_contracts(
            ...
            theme="Core",
            epic="Testing",
            invest_validation_expected=False,  # Validation was disabled
        )
        
        # Should pass - validation result is present (bonus)
        assert result.is_valid is True
        assert result.validation_skipped is False
        assert len(result.violations) == 0

class TestEnforceInvestResultPresence:
    """Direct unit tests for enforce_invest_result_presence function."""

    def test_missing_result_expected_returns_violation(self):
        """When validation_result is None and expected, should return violation."""
        violation = enforce_invest_result_presence(
            validation_result=None,
            invest_validation_expected=True,
        )
        assert violation is not None
        assert violation.rule == "INVEST_RESULT_MISSING"

    def test_missing_result_not_expected_returns_none(self):
        """When validation_result is None but NOT expected, should return None (no violation)."""
        violation = enforce_invest_result_presence(
            validation_result=None,
    ...
    def test_present_result_with_score_returns_none(self):
        """When validation_result has a valid score, should return None (no violation)."""
        violation = enforce_invest_result_presence(
            validation_result={"validation_score": 85},
            invest_validation_expected=True,
        )
    ...
    def test_result_missing_score_returns_violation(self):
        """When validation_result exists but has no score, should return violation."""
        violation = enforce_invest_result_presence(
            validation_result={"some_other_field": "value"},
            invest_validation_expected=True,
        )
        assert violation is not None
        assert violation.rule == "INVEST_SCORE_MISSING"

    def test_result_with_zero_score_returns_violation(self):
        """When validation_result has score=0, should return violation."""
        violation = enforce_invest_result_presence(
