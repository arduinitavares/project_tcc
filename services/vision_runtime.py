"""Runtime helpers for invoking the product vision agent from workflow state."""
...
type VisionInputContext = dict[str, str]
type ValidationErrors = list[dict[str, object]]


@dataclass(frozen=True)
class _FailureDetails:
    """Structured details describing a vision-runtime failure."""
...
def _normalize_prior_vision_state(value: object) -> str:
    if value is None:
        return "NO_HISTORY"
    if isinstance(value, str):
...
def build_vision_input_context(
    ...
    *,
    ...
    """Build the serialized vision-agent input payload from workflow state."""
    return {
...
def _failure(
    *,
    project_id: int,
    input_context: VisionInputContext,
    failure_stage: str,
...
async def run_vision_agent_from_state(
    ...
    *,
    ...
    """Run the vision agent from stored workflow state and normalize failures."""
    input_context: VisionInputContext = build_vision_input_context(
    ...
    try:
        raw_text: str = await _invoke_vision_agent(payload)
