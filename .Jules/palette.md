...
## 2023-10-24 - Required Form Field Indicators
**Learning:** Adding the `<span class="text-red-500" aria-hidden="true">*</span>` indicator and semantic `required` attribute directly to modal forms greatly improves explicit user expectations before form validation triggers an alert. The visual asterisk pairs perfectly with existing Tailwind label components.
**Action:** Always include a visual required indicator and semantic attribute on any inputs that are implicitly required by Javascript validation to ensure accessibility and clear UX expectations.
