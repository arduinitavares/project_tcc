...
- [`orchestrator_agent/agent_tools/user_story_writer_tool/scrum_theory_user_story_writer_tool.md`](../../../orchestrator_agent/agent_tools/user_story_writer_tool/scrum_theory_user_story_writer_tool.md)
- [`orchestrator_agent/agent_tools/sprint_planner_tool/scrum_theory_sprint_planner.md`](../../../orchestrator_agent/agent_tools/sprint_planner_tool/scrum_theory_sprint_planner.md)
- [`orchestrator_agent/agent_tools/spec_authority_compiler_agent/scrum_theory_spec_authority.md`](../../../orchestrator_agent/agent_tools/spec_authority_compiler_agent/scrum_theory_spec_authority.md)
...
## Task Checklist Production

Task checklists should be planner-generated by default, with optional human refinement before save.

### Default Producer

The Sprint Planner should be the default producer of `task checklist` items when it emits decomposed task objects.

This is the most natural place to generate them because the planner already has the needed context:

- story acceptance criteria
- story description
- task description
- artifact targets
- task-local invariant bindings

For implementation planning purposes, the intended evolution is:

- extend structured task output with `checklist_items`
- keep checklist generation in the same draft -> review -> save lifecycle already used for sprint planning artifacts

### Human Refinement

Users may refine planner-generated checklist items during sprint review before save.

This refinement is optional, not required for normal operation. The platform should remain useful even when no manual edits are made.

### Production Guardrails

Task checklist items must describe **task-local completion conditions or observable outcomes**, not story-level completion.

The planner should be instructed explicitly:

> Task checklist items must describe task-local completion criteria. Do not copy or paraphrase story acceptance criteria into task checklists.

The checklist should stay concise. As a default heuristic:

- usually 1-4 items per task
- focus on outcomes/evidence, not fine-grained coding instructions
- avoid turning the checklist into a mini implementation plan

### Deterministic Validation

The quality gate should validate planner-produced checklist items deterministically.

Recommended validation behavior:

- reject exact or normalized duplicates of story acceptance criteria
- reject broad story-completion phrasing such as "complete the story" or "satisfy all acceptance criteria"
- warn on high lexical overlap with story acceptance criteria

Semantic similarity matching may be added later, but it should not be a hard dependency for v1.

### Empty Checklist Fallback

If the planner emits an empty checklist, the platform must not synthesize one from story acceptance criteria.

Instead, the task should be treated as either:

- informational/reference-only, or
- a candidate to be folded into another task as checklist items

If a task is presented as copyable/executable, it should have at least one checklist item.
...
`Copy Story Prompt` is a net-new platform capability. It is not just a small variation of the existing task-level prompt renderer.
...
To reduce user error, the prompt itself should include a compact warning such as:
...
### Operational Note on Long Sessions

Long-running agent sessions may gradually lose earlier context. For stories with many sequential task prompts, the platform should document an operational guideline:

- if the session becomes long or drift-prone, start a new session with `Copy Story Prompt` and continue from there

This is workflow guidance, not a requirement to make task prompts self-contained.
...
If a parent-story reference block is rendered, it should contain only compact orientation data such as:

- story title
- story statement/description summary

It should not include the full story acceptance criteria text, because that would reintroduce scope confusion in the task prompt.

### Static Renderer Text Changes

The renderer's fixed task-prompt instructions are part of this contract change.

Under this design, task-oriented renderer text should use task-scoped terminology, for example:

- `Task Checklist` instead of `Acceptance Criteria Checklist`
- "Verify every task checklist item before claiming completion"

Story-oriented renderers may continue to use story acceptance criteria terminology where appropriate.
...
For initial implementation, this evidence model should align with the existing task execution log concepts already present in the codebase, including:
...
If later work introduces checklist-item-level evidence, that should be treated as an extension beyond the initial task-level logging model.
...
| Term | Scope | Answers |
| :--- | :--- | :--- |
| `definition of done` | Cross-cutting quality bar | "What standards apply broadly?" |
| `story acceptance criteria` | Story completion | "When is the story done?" |
| `task checklist` | Task completion | "What must happen for this execution slice to be done?" |
